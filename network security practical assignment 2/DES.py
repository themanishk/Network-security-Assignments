...
def hamming_dis(fi,se):                
        dis=0
        k=len(se)
        for i in range(k):
            if se[i]!=fi[i]: dis=dis+1
        return dis
    
def get_lis(k,pt,key):                    
    lis=[]                            
    for plaintext in pt:
...
# quesn i)
#  pti[0] is the reference plaintext, HD=1 of pti[0] w.r.t other pti[i]. i=1,2,..5
pti=[b'qwertyaa',b'qwertqaa',b'qgertyaa',b'awertyaa',b'qwerfyaa',b'qwqrtyaa']
print('\nplaintext              ciphertext                         key')
get_plot(get_lis(k,pti,b"01hcukvc"),"5 different plain text with hamm dist=1",0,1)

# quesn ii 
# Hds ptii[i]  are 1,2,3,4,5 w.r.t ptii[0].
ptii=[b'qwertyaa',b'qwvstqaa',b'qaertya#',b'awertyqa',b'qwdafhkl',b'afqp#yaz']
print('\nplaintext              ciphertext                         key')
get_plot(get_lis(k,ptii,b"01hcukvc"), \
         "5 different hamming distances",1,1)

# quesn iii)
# The first string of k_list is the reference key others have a HD of one w.r.t to this reference key
k_list=[des(b'qwertyaa'),des(b'qwertqaa'),des(b'qgertyaa'),des(b'awertyaa'),des(b'qwerfyaa'),des(b'qbcdefgh')]
k_list1=[b'qwertyaa',b'qwertqaa',b'qgertyaa',b'awertyaa',b'qwerfyaa',b'qbcdefgh']
plaintext=b"01hcukvc"
...
get_plot(lis,"5 differentsecret keys",0,0)